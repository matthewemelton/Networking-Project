...
def Upload(fileName, connection):
  ...
  #   )
    ...
    # print(f"Current file bytes: {fileBytes}")
    print("Attempting to add file to server")
    fileStorage[fileName] = File(fileName)
    fileObj = fileStorage[fileName]
    fileObj.fileBytes = bytes(fileBytes)
    print(fileObj.fileBytes[:12], fileObj.fileBytes[-4:])
