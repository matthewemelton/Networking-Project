...
import socket, os, time, datetime, threading
...
def UploadReqFromServer(s):
  print("MADE IT REQUESTED FROM SERVER")
  data = s.recv(1024)  # Receive data from the server
  data = data.decode(FORMAT)
  transmission = data.split()

  print(f"Transmission {transmission}")
  if transmission[0] == "CHECK":
    fileName = transmission[1]
    Upload(fileName)

  s.send(b"FILE SENT".encode(FORMAT))
...
if __name__ == '__main__':
  ...
  t1 = threading.Thread(target=UploadReqFromServer, args=[s])
  t2 = threading.Thread(target=Main, args=[s])
