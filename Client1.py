...
def Scenario2_2(file1, file2):
  ...
  # recieve the first file
  with open(f"./Client1Files/Merged", "wb") as merged:
    frame = s.recv(CHUNK)
    while len(frame) == CHUNK:
      merged.write(frame)
      frame = s.recv(CHUNK)
  ...
  with open("./Client1Files/Merged", "rb") as merged:
      with open(f"./Client1Files/{file1}", "wb") as m1:
        m1.write(merged.read(splitPoint))
      with open(f"./Client1Files/{file2}", "wb") as m2:
